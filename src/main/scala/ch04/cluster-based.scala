...
object ClusterBasedRecommenderTest extends App {
  ...
  lazy val recommenderBuilder = new RecommenderBuilder {
    def buildRecommender(model: DataModel) = {
      ...
      //val clusterSimilarity = new FarthestNeighborClusterSimilarity(similarity)
      val clusterSimilarity = new NearestNeighborClusterSimilarity(similarity)
